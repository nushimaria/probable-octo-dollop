...
$( document ).ready(function() {
    ...
    myForm.addEventListener("submit", function (e) {
      ...
      reader.onload = function (e) {
        ...
        data.forEach(function(x){
            let first = x.First.trim();
            first = first.replace(/(\r\n|\n|\r)/gm, "").replace(/\s/g,"");
            let last = x.Last.trim();
            last = last.replace(/(\r\n|\n|\r)/gm, "").replace(/\s/g,"");
    ...
    myForm2.addEventListener("submit", function (e) {
      ...
      reader.onload = function (e) {
        ...
        console.log(data);
        data.forEach(function(x){
            let email = x.Email.trim();
            email = email.replace(/(\r\n|\n|\r)/gm, "").replace(/\s/g,"");
            add_custom_potential_email(email)
        });
      };
...
function add_custom_potential_email(email) {
    if (!email) {
        var pemail = prompt("Enter potential email you'd like to add to the list:", "");
    }else if (email) {
        let append_htmlz = get_row_html(email);
        $('#results_tbody').append(append_htmlz);
    }else if (pemail) {
        let append_htmlz = get_row_html(pemail);
        $('#results_tbody').append(append_htmlz);
    }
}
...
function check_gmail(email) {
    ...
    //for(var i=0; i<emails.length; i++) {
        ...
        */
        $.ajax({
            type: 'GET',
            url: "https://mail.google.com/mail/gxlu?email="+email+"&tab_id="+my_tab_id,
            success: function(output, status, xhr) {
...
function check_o365(email) {
    ...
    //for(var i=0; i<emails.length; i++) {
        ...
        */
        var o365username = email.replace(/\./g, "_").replace('@', '_');
        var o365username_domain = email.split("@")[1].split('.');
...
function check_github(email) {
    ...
    //for(var i=0; i<emails.length; i++) {
        $.ajax({
            type: 'GET',
            url: "https://api.github.com/search/commits?q=committer-email:"+email,
...
function trigger_scan() {
    ...
    var x = setInterval(function(){
        if (counter == emails.length) {
            ...
            check_gmail(emails[counter]);
            check_o365(emails[counter]);
            check_github(emails[counter]);
            counter++;
            document.getElementById("progress").innerText = "Scanning " + counter + "/ " + emails.length;
        }
